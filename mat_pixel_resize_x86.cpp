...
#if _x86_INTRIN
        __m64 _b0 = _mm_broadcast_epi64(b0);    // 需要修改，用set指令
        __m64 _b1 = _mm_broadcast_epi64(b1);    // 需要修改，用set指令
        __mm128i _v2 = _mm_broadcast_si128(2);
        for(; nn > 0; nn--)
        {
            __m64 _rows0p_sr4 = _mm_loadl_epi64(rows0p);
            __m64 _rows1p_sr4 = _mm_loadl_epi64(rows1p);
            // __mm128i _rows0p_1_sr4 = _mm_load_epi64(rows0p + 4);
            // __mm128i _rows1p_1_sr4 = _mm_load_epi64(rows1p + 4);

            __mm128i _rows0p_sr4_mb0 = _mm_mullo_epi16(_rows0p_sr4, b0);
            __mm128i _rows1p_sr4_mb1 = _mm_mullo_epi16(_rows1p_sr4, b1);
            // __mm128 _rows0p_1_sr4_mb0 = _mm_mullo_epi16(_rows0p_1_sr4, _b0);
            // __mm128 _rows1p_1_sr4_mb1 = _mm_mullo_epi16(_rows1p_1_sr4, _b1);

            // 右移指令
            __mm128i _acc = _v2;
            _acc = _mm_add_epi32(_mm_srai_epi32(_rows0p_sr4_mb0, 16), _acc);
            _acc = _mm_add_epi32(_mm_srai_epi32(_rows1p_sr4_mb1, 16), _acc);
            
            // 右移指令
            __m128i _acc16 = _mm_srai_epi32(_acc, 2);

            _mm_storel_epi64(Dp, _acc);
